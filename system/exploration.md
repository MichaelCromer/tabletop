...
Here is a sample list of Exploration actions that players can take. You will almost certainly need to add or remove actions based on your specific scenario, and also be prepared to adjudicate unlisted actions the players would like to attempt.

| Action | Description |
|---|---|
| Conceal | Hide in place or prepare to move undetected. |
| Disable | Disarm a trap, pick a lock, or negate a hazard. |
| Interact | Manipulate an object, mechanism, or device in the environment. |
| Move | Proceed to an adjacent room or area. |
| Scout | Scan the current or adjacent area for visible threats, features, and points of interest. |
| Search | Conduct a thorough inspection for hidden features, traps, or treasure. |
| Study | Carefully examine a puzzle, magical effect, or complex phenomenon. |
| Wait | Hold still and do nothing for the turn. |
...
Movement is for when players wish to proceed to an adjacent room or area within the Location.

A player Moves by indicating which direction they wish to go. If nothing bars their way, this automatically succeeds and takes one Exploration turn. If a guard or other threat blocks the path, the player may Conceal before Moving to slip past unnoticed, or use Disable or Interact to remove the obstacle before they can Move through.
...
Waiting is for when players wish to pass the turn without taking any significant action.

A player Waits by announcing their intention to do nothing: keeping watch, staying quiet, or simply biding their time. Waiting automatically succeeds and takes one Exploration turn. A concealed player who Waits remains concealed; the Conceal check carries over until they take a different action.
...
| Event | Description |
|---|---|
| Disturbance | A distant noise, tremor, or sign of activity elsewhere in the Location. |
| Environmental Shift | Lights dim, the temperature changes, or a breeze stirs the air. |
| Sign of Passage | Fresh tracks, displaced objects, or other evidence of recent movement. |
| Ominous Sign | An unsettling omen — a symbol, a smell, a sound — that hints at the Location's nature. |
| The Place Reacts | A door creaks, dust falls from the ceiling, or water drips somewhere nearby. |

Major Event (2+ sixes)
| Event | Description |
|---|---|
| Wandering Threat | A patrol or creature moves into the players' current area. |
| Location Shift | A passage seals, a new route opens, or the layout rearranges itself. |
| Trap Resets | A previously triggered trap re-arms or a hazard escalates in severity. |
| Reinforcements | Hostile forces in a nearby area are alerted to the players' presence and begin to act. |
| Environmental Hazard | A cave-in, flood, gas leak, or other significant environmental change occurs. |
| Timer Advances | A puzzle, ritual, or narrative clock ticks forward, narrowing the players' window of opportunity. |

### Traps

Traps are hazards that trigger when a creature enters an area or interacts with an object. They may be mechanical (pit, dart, swinging blade), magical (explosive rune, binding ward), or environmental (collapsing floor, toxic gas).

Players can detect a trap through an Observe action (noticing obvious signs) or a Search action (finding well-hidden mechanisms). They may disable it with a Disable action, bypass it through another creative approach, or simply trigger it and deal with the consequences.

The severity of a trap depends on the Location. A simple snare might inflict a condition, while a magical ward could deal lasting Wounds or teleport the victim to a dangerous part of the dungeon. Use your judgement to set appropriate difficulties and consequences.

### Puzzles

Puzzles are challenges that require creative thinking, pattern recognition, or specific knowledge to overcome. They may be physical (matching symbols, pressure plates), logical (riddles, lock-and-key arrangements), or magical (alignment of celestial seals, recitation of forbidden words).
...
When designing a hazard, consider: what is the direct consequence of failure (damage, condition, resource loss), what actions can resolve or mitigate it, and how does it interact with the Location's other features?
